...
void white(Piece& piece, int xx, int yy){
    if(pieceMap[piece.x][piece.y].front().no != piece.no) // 현재 말이 가장 밑에 있는 말이 아니면 로직 자체를 안 탐
        return;
    
    vector<Piece> tempV(pieceMap[piece.x][piece.y].begin(), pieceMap[piece.x][piece.y].end()); //현재 벡터를 복사
    pieceMap[piece.x][piece.y].clear(); // 현재 벡터 삭제
